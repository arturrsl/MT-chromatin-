...
% quantitative analysis of chromatin fibers under tension

% based on: Meng, Andresen, van Noort - NAR, 2015

% input parameters: force ramp (pN),contour length (nm), number of
% nucleosomes, Nucleosome Repeat Length, fiber stiffness, degeneracy, free
% energy of unstacking (dG1), free energy of the intermediate transition (dG2)
...
for i= 1:(Nnuc+1)
    ...
    for j=1: (Nnuc - i +3)  
        ...
        for k = 1: (Nnuc - i - j +3)
               ...
               D(s) = factorial(Nnuc)./(factorial(i-1).*factorial(Nnuc-(i-1))) .* (factorial(Nnuc-(i-1))./(factorial(j-1).*factorial(Nnuc-(i-1)-(j-1)))) .* (factorial(Nnuc-(i-1)-(j-1))./(factorial(k-1) .*factorial(Nnuc-(i-1)-(j-1)-(k-1)))); % this particular state exists in D combinations to be at
               ...
               z_tot(s,:) = z_wrapped + state(s,1) .* z_fiber + state(s,2) .* z_singlewrap + state(s,3) .* z_extended + state(s,4) .* z_unwrapped;          % extension of the subunit  multiplied by the amount of subunits in this particular step
...
for i = 1:length(f)
    G_total(:,i) = G_tot(:,i) - min(G_tot(:,i));                            % offseting the absolute values of energies to avoid large negative numbers that go to exponent; they serve only for Boltmann weighing so it's fine ;]
end
